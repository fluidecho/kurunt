...
function _load() {
	...
	if ( config['run_standalone'] === true ) {
		// copyright statement.
		console.log('Welcome to Soci Server (input) (http://kurunt.org).\nVersion '+version+' (Apache License 2.0).\n\nCopyright (c) 2012 Mark W. B. Ashcroft.\nCopyright (c) 2012 Kurunt.\n\nType ctrl+c to exit the program.\n>>>');
	} else {
		console.log('*Soci input started.');
	}

	
	
	handle["*"] = handlers.all;										// allow clients from any host and any open port handler - see README for more info.
	//handle["10.10.10.10"] = handlers.host_10_10_10_10;			// example client host specific handler, disable handle["*"].
...
function startServer(ports) {
	...
	if ( ports == '' ) {
		console.log("Soci (input) '" + config['name'] + "' has no ports to open, goodbye.");
		if ( config['run_standalone'] === true ) {
			process.exit(0);  										// exit
		}
