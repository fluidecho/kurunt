...
if ( $h ne '' || $help ne '' ) {
	...
	print "   -m = 10 (number, required command, of messages to send)\n";
	print "   -c = 10 (number, required command, of cycles to send messages)\n\n";
	print "For example> perl benchmark.pl -T=tcp -P=7001 -m=10 -c=10\n";
	print "This example would send 100 messages every second for 10 seconds through port 7001, -H (host)\n";
	exit(0);
}
if ( $P eq '' ) {
