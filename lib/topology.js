...
function topo(config, topology) {
	...
	//for ( x = 0; x < topology['nodes'].length; x++ ) {
		...
		//	processes[f] = require( __dirname + '/' + object );
			...
			processes[f].send({ id: p, config: JSON.stringify(config), topology: JSON.stringify(topology['nodes'][config['this_node']]['process'][p]) });
