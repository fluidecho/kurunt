...
module.exports._new = function (webadmin, config, processes, input, worker, stores, tags, access_hosts, newstreamCB) {
	...
	if ( input === 'tcp' || input === 'udp' || input === 'tls' ) {
		last_input_port++;
	...
	newstream.dynamic = true;		// was created via module not using web admin (streams.json).
	...
	if ( webadmin.host_address != undefined ) {
		newstream.host = webadmin.host_address;
	} else {
		newstream.host = config['host'];
	}
	
	if ( input === 'http' ) {
		newstream.address = input + '://' + newstream.host + ':' + httpconfig['input_port'] + '/' + apikey + '/';
	} else if ( input === 'tcp' ) {
		newstream.address = input + '://' + newstream.host + ':' + apikey;
	} else if ( input === 'udp' ) {
		newstream.address = input + '://' + newstream.host + ':' + apikey;
	}

	newstream.api_address = 'http://' + apikey + ':' + api_config['stream_api_pass'] + '@' + newstream.host + ':' + api_config['stream_api_port'];


	//console.log('newstream.js> processes: ' + require('util').inspect(processes, true, 99, true));
	...
	//logging.log('NewStream, set in web admin dtreams.');
	webadmin.dstreams['streams'].push(newstream);
