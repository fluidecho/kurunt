...
process.on('SIGINT', function() {
	log(processID + '@stores> SIGINT, exit.');
  process.exit(code=0);
});
...
process.on('message', function(m) {
	log('*stores, process.on.message>', false, m);
	if ( m.id !== undefined ) {
		processID = '*#' + m.id;
		id = m.id;
	}
...
process.argv.forEach(function(val, index, array) {
	...
	if ( val.substring(0, 8) === '-stores=' ) {
		...
		log('stores> load these stores: ' + stores_asmodules);
		log('stores> store: ', false, stores_asmodules);
	}
...
function _init(config, topology) {
	...
	log(processID + "@stores> initiating.");
	...
	setInterval(function () {		
		log(processID + '@stores> mps: ' + mps + ' total: ' + tot, true);
		mps = 0;		// reset
	}, 1000);	
...
function _loadAllStores() {
	...
	for ( var st in stores_asmodules ) {
		...
		if( typeof stores[store_config.name]['module'].init === 'function' ) {
			stores[store_config.name]['module'].init();
		}			
		...
		dirs.forEach(function(report){
			...
			if (stats.isDirectory()) {
				...
				if ( report.substring(0, 1) != '_' ) {
					...
					log('*@stores> report to load: ' + this_config['title']);
