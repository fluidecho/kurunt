...
*/
	...
	console.log('stream:messenger@stores> connections: ' + util.inspect(connections, true, 99, true));
	var c = 0;
	for ( c = 0; c < connections.length; c++ ) {
		console.log('stream:messenger@stores> make ZMQ this connection!!!');
		...
		console.log('stream:messenger@stores> zmq_pattern: ' + zmq_pattern );		
		
		
		socks[c] = mq.socket(zmq_pattern);
		console.log('stream:messenger@stores> zmq_address: ' + connections[c]['zmq_address']);
 		socks[c].connect(connections[c]['zmq_address']);
		console.log('stream:messenger@stores> messenger, connected: ' + zmq_pattern + '@' + connections[c]['zmq_address']);		
	...
	socks[c].on('connect', function(a){
		console.log('stream:messenger@stores> CONNECTED --------------');
		var _this = this;
