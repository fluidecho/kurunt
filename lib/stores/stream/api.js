	...
	if ( lconfig['stream_api'] === true ) {
		...
		for ( var client in clients ) {
		  ...
		  if ( message.apikey.toString() === clients[client]['channel'] || message.tags.indexOf(clients[client]['channel']) != -1 || clients[client]['channel'] === 'all' ) {
		    logging.log('api:stream@stores> send message to clientID: ' + clients[client]['id'] + ' for channel: ' + clients[client]['channel']);
		    clients[client]['res'].write( JSON.stringify(message) + '\n' );   // delineate by linefeed.
		  } else {
		    logging.log('api:stream@stores> skip invalid channel.');
		  }       
		}
	}  
...
function onRequest(request, response) {
  ...
  // validate password is either: pass or lconfig['streaming_api_password'].
  if ( channel != 'all' && lconfig['stream_api_pass'] != auth_password || (lconfig['stream_api_all_pass'] != auth_password && channel === 'all') ) {
  ...
  if ( channel === 'all' && auth_password != lconfig['stream_api_all_pass'] ) {
    response.writeHead(401, {'WWW-Authenticate': 'Basic realm="Kurunt Stream API"', 'Content-Type': 'application/json', 'Connection': 'closed'});
    response.end('{"message":"not valid auth"}\n');
    return;
  } 
  ...
  logging.log('api:stream@stores> Valid user, username is "'+auth_username+'" and password is "'+auth_password+'"');  
  logging.log('api:stream@stores> Client connect. Last request for: ' + request.url + ' by ip: ' + ip + ' clientID: ' + clientID);

  request.connection.addListener('close', function () {
    logging.log('api:stream@stores> Client closed. Last request for: ' + request.url + ' by ip: ' + ip + ' clientID: ' + clientID);
...
if ( lconfig['stream_api'] === true ) {
  server = http.createServer(onRequest).listen(lconfig['stream_api_port']);      // Start a HTTP Server.
  logging.log("api:stream@stores> Stream API Server has opened on port " + lconfig['stream_api_port'] + ".");
} else {
  logging.log("api:stream@stores> Stream API is closed.");
}
