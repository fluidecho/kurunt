...
module.exports.message = function (message) {
	...
	for ( var client in clients ) {
		...
		if ( message.apikey.toString() === clients[client]['channel'] || message.tags.indexOf(clients[client]['channel']) != -1 ) {
			console.log('api:stream@stores> send message to clientID: ' + clients[client]['id'] + ' for channel: ' + clients[client]['channel']);
			clients[client]['res'].write( JSON.stringify(message) + '\n' );		// delineate by linefeed.
		} else {
			console.log('api:stream@stores> skip invalid channel.');
		}				
	}
};
