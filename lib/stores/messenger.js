...
function init(xconfig, Connections) {
	...
	console.log('messenger@stores, connections> ' + util.inspect(connections, true, 99, true));
	var c = 0;
	for ( c = 0; c < connections.length; c++ ) {
		console.log('messenger@stores> make ZMQ this connection!!!');
		...
		console.log('messenger@stores> zmq_pattern: ' + zmq_pattern );		
		
		socks[c] = mq.socket(zmq_pattern);
		console.log('messenger@stores> zmq_address: ' + connections[c]['zmq_address']);
		if ( connections[c]['zmq_socket'] === 'bind' ) {
 			socks[c].bind(connections[c]['zmq_address']);
 		} else {
 			socks[c].connect(connections[c]['zmq_address']);
 		}
		console.log('messenger@stores> messenger, ' + connections[c]['zmq_socket'] + ':' + zmq_pattern + '@' + connections[c]['zmq_address']);			

		//socks[c].format('json');

		// SET this from connected axon socket so can control pause() and resume().
		socks[c].on('connect', function(a){
			console.log('messenger@stores> CONNECTED --------------');
			var _this = this;
