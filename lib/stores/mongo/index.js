...
module.exports.init = function (processID, cf, fn) {
	...
	try {
		...
		MongoClient.connect('mongodb://' + config['mongo_host'] + ':' + config['mongo_port'] + '/' + config['dbname'], function(err, db) {
		    if(err) throw err;
		    _db = db;
		    fn.logging.log('mongo@stores> db connected.');
		});
	} catch(e) {
		fn.logging.log('mongo@store> To use the MongoDB store you\'ll need to install the mongodb module: npm install mongodb -g');
	}
...
module.exports.store = function (message, fn, callback) {
  ...
  // See: http://docs.kurunt.com/Mongo_Store
	...
	//console.log('mongo@stores> MESSAGE: ' + require('util').inspect(message, true, 99, true));
	
  fn.logging.log('mongo@stores, MESSAGE> ', message);
  ...
  try {
    ...
    fn.logging.log('mongo@stores, ERROR> ', e);
    return callback( false );
  }
