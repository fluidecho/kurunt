...
module.exports.init = function (processID, cf, fn) {
	...
	try {
		...
		MongoClient.connect('mongodb://' + config['mongo_host'] + ':' + config['27017'] + '/' + config['dbname'], function(err, db) {
		    if(err) throw err;
		    _db = db;
		    logging.log('mongo@stores> db connected.');
		});
	} catch(e) {
		logging.log('mongo@store> To use the MongoDB store you\'ll need to install the mongodb module: npm install mongodb -g');
	}
