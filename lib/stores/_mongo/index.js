...
try {
	...
	console.log('mongo@store> You need to install the mongodb module: npm install mongodb');
}
...
module.exports.store = function (message, report, callback) {
  ...
  // See: http://docs.kurunt.com/store/mongo/

  //console.log('mongo@stores, MESSAGE> ' + require('util').inspect(message, true, 99, true));    // uncomment to debug message.
  ...
  try {
  	...
  	// NOTE: you can write your mongoDB store anyway you prefer!
		...
		//console.log('mongo@stores, data> ' + require('util').inspect(data, true, 99, true));    // uncomment to debug.
		if ( data === undefined ) {		
			return callback( false );
		}
