...
try {
	...
	log('redis@store> To use the Redis store you\'ll need to install the redis module: npm install redis');
}
...
module.exports.store = function (message, report, callback) {
  ...
  // See: http://docs.kurunt.com/stores/redis/
  ...
  try {
  	...
  	// NOTE: you can write your redis store anyway you prefer!
		...
		var collectionName = message.apikey.toString() + ':' + message.id;	
