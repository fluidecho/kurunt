...
function init(config, topology, workers, stores, callback) {
	...
	Topo.topo(config, topology, workers, stores, function(xprocesses) {
		//console.log('init.js> xprocesses: ' + require('util').inspect(xprocesses, true, 99, true));
		return callback( xprocesses );		// returns processes falked.
