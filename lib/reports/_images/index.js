...
var STREAM_REPORT_PORT = 9000;
app.listen(STREAM_REPORT_PORT);


// kurunt messages.
var Messages 		= require('../../../lib/stores/./index.js');
...
messages.on('message', function(message, reply) {
	...
	for ( var s in message.stores ) {
		...
		for ( var v in message.stores[s] ) {
			...
			if ( v === 'stream' ) {
				for ( var d in message.stores[s]['stream']['schema'] ) {
					...
					//console.log("value size: " + value_json.length + " characters, " + Buffer.byteLength(value_json, 'utf8') + " bytes");
					
					
		// TESTING ---------
		// 1: convert js buffer array to node buffer.
		var value_buf = new Buffer(value);
		...
		message.stores[s]['stream']['schema'][d]['value'] = '...';		
		message['image'] = mypic;		
