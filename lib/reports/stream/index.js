...
try {
	...
	console.log(pname + ' You need to install socket.io. >npm install socket.io');
}
...
module.exports._init = function (processID) {
	...
	if ( io != undefined ) {
		...
		log('stream#' + processID + '@reports> opening webserver on port: ' + STREAM_REPORT_PORT);
		pname = 'stream#' + processID + '@reports>';
		app.listen(STREAM_REPORT_PORT);
	} else {
		log('stream#' + processID + '@reports> not opening, socket.io not installed.');
	}
};
      ...
      socket.join(stream);
			...
			if (timmer === undefined ) {
				timmer = setInterval(function () {
					...
					log('stream@reports> (all) mps: ' + mps['all'] + ' n: ' + tot['all'], true);
