...
var MESSAGES_QUE_X = 10;		// how many (number) messages to queue. 
var messages_que = [];
...
if ( io != undefined ) {
	...
	io.sockets.on('connection', function (socket) {
    	...
    	console.log('getStream req: ' + stream);
  		socket.set('stream', stream, function() { console.log('stream ' + stream + ' saved'); } );
      socket.join(stream);
				...
				timmer = setInterval(function () {
					...
					for ( var r in io.sockets.manager.rooms ) {
						...
						if ( r != '' ) {
							...
							io.sockets.in(room).emit('ticker', tot, mps);
							//if ( messages_que.length > 0 ) {
							//}
							io.sockets.in(room).emit('messages', messages_que);					
						}
