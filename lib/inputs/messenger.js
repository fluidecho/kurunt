...
// Messenger (inputs)
//
// Queue and send input (TCP, UDP, HTTP) messages to Kurunt workers.
// Version: 0.2
...
function init(processID, topology) {
	...
	for ( var c = 0; c < topology['connections'].length; c++ ) {
		...
		if ( topology['connections'][c]['zmq_socket'] == 'bind' ) {
			console.log("zmq binding for apikey: " + apikey + ", namespace: " + namespace + ", at: " + zmq_pattern + "@" + zmq_address);
			mq_sockets[c]['socket'].bind(topology['connections'][c]['zmq_address']);
		} else {
			console.log("zmq connecting for apikey: " + apikey + ", namespace: " + namespace + ", at: " + zmq_pattern + "@" + zmq_address);
			mq_sockets[c]['socket'].connect(topology['connections'][c]['zmq_address']);	
		}
		
		mq_sockets[c]['socket'].on('disconnect', function(s){
			console.log("disconnected: " + s._peername.port);
	...
	setInterval(function () {		
		...
		console.log('*#' + processID + '@input> mps: ' + mps + ' total: ' + tot);
		mps = 0;		// reset
	}, 1000);
...
function push(apikey, input, worker, stores, tags, client, chunk) {

	//console.log('m: ' + Buffer.isBuffer(chunk) + ' len: ' + chunk.length +  ' messSt: ' + chunk.toString('ascii'));
	...
	for (i = 0; i < bufferLoop.length; i++) {
  		...
  		if ( bufferLoop.readUInt8(i) == config["input_tcp_message_delineate"] && i != (esc + 1) ) {
  			//console.log('is delineate, at: ' + i + ' x: ' + x + ' messages.length: ' + messages.length);
				...
				if ( config['mq_nodelay'] === true ) {
					...
					send(msgs, apikey, input, worker, stores, tags, client, buffer_size, false);
						...
						nagleishTimeOut = (function () {
							setTimeout(function () {
								if ( buffer_size > 0 ) {
									...
									messages = [];
									buffer_size = 0;
									send(msgs, apikey, input, worker, stores, tags, client, buffer_size, true);				// send.
								}
							}, TCP_TRAIL_EXPIRES);
						})();	
					} 			
					...
					if ( buffer_size > TCP_BUFFER_SIZE ) {
							...
							messages = [];
							buffer_size = 0;
							send(msgs, apikey, input, worker, stores, tags, client, buffer_size, false);							// send.
					}
...
function send(m, apikey, input, worker, stores, tags, client, buffer_size, timedOut) {
	...
	for ( var c in mq_sockets ) {
		if ( mq_sockets[c]['apikey'] == '*' || mq_sockets[c]['apikey'] == apikey ) {
			...
			//console.log('sending from client ' + client['address']+':'+client['port'] + ' for apikey: ' + apikey + ' on socket apikey: ' + mq_sockets[c]['apikey']);
...
//console.log('msg> ' + util.inspect(msg, true, 99, true));
			...
			mq_sockets[c]['socket'].send(msg, function(rep){
				//console.log('replied: ' + rep);
				//console.log('rep> ' + util.inspect(rep, true, 99, true));
				//var mdcodex = codec.decode(msg);
				//console.log('r> ' + util.inspect(mdcodex, true, 99, true));
			});			
