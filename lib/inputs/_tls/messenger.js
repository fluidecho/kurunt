...
function init(_pid, topology) {
	...
	for ( var c = 0; c < topology['connections'].length; c++ ) {
		...
		mq_sockets[c]['socket'].format('json');				// TODO: no codec in axon 2.*
...
function push(apikey, chunk) {
	...
	//console.log('chunk buff limti: ' + lconfig['chunk_buffer_limit']);
	...
	if ( buffer.length > lconfig['chunk_buffer_limit'] ) {
		...
		logging.log('messenger:tls@inputs> Warning: Maximum chunk buffer size limit exceeded, dropping buffer.');
		buffer = null;
		return false;
	}
	...
	for (i = 0; i < bufferLoop.length; i++) {
  		...
  		if ( bufferLoop.readUInt8(i) == lconfig["message_delineate"] && i != (esc + 1) ) {
  			logging.log('tls@inputs> is delineate, at: ' + i + ' x: ' + x + ' messages.length: ' + messages.length + ', for apikey: ' + apikey);
				...
				logging.log('tls@inputs> m: ' + m.toString());
				
				logging.log('tls@inputs> m.length: ' + m.length);
				if ( m.length === 0 ) {
					// cant send a blank event, return nada.
					logging.log('tls@inputs> cant send a blank event, return false');
					return false;
				}
				...
				logging.log('tls@inputs> buffer_size[apikey]: ' + buffer_size[apikey]);
				if ( gconfig['mq_nodelay'] === true ) {
					...
					if ( messages[apikey].length === 1 ) {
						logging.log('tls@inputs> nagleishTimeOut');
						// start nagleish delay timmer.
						nagleishTimeOut[apikey] = (function () {
							setTimeout(function () {
								logging.log('tls@inputs> nagleishTimeOut callbacked');
								if ( buffer_size[apikey] > 0 ) {
									logging.log('tls@inputs> nagleishTimeOut callbacked, if buffer_size[apikey] > 0');
									var msgs = messages[apikey];
...
function send(m, apikey, buffer_size, timedOut) {
	...
	try {
		if ( streams[apikey].status === 'closed' ) {
			logging.log('messenger:tls@inputs> stream closed dont send');
			return false;
		}
	} catch(e) {
		return false;
	...
	logging.log('messenger:tls@inputs> send');
	...
	logging.log('messenger:tls@inputs> apikey: ' + apikey, streams[apikey]);
