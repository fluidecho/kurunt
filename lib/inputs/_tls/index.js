...
// TLS Input
//
// TLS input server for tcp secured clients.
// Version: 0.2
...
var version 			= 0.2;																		// tls input version number.

var server 				= require("./server");										// the tls (tcp) server.
...
function startServer(pid, streams) {
	if ( streams == undefined ) {
		logging.log(processID + "@inputs> TLS (input) '" + config['name'] + "' has no streams to open, goodbye.");
		streams = {};
		streams.streams = [];
	}
	server_started = true;
	...
	process.send({ object: 'input', namespace: 'tls', message: 'loaded' });
}
