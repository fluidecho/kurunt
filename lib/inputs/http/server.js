...
*/
			...
			// route ...
			route(apikey, config['name'], req.headers.host, message, id, mime_type, messenger, streams, function (validHost) {
				if ( validHost === true ) {
				    ...
				    res.end('\x47\x49\x46\x38\x39\x61\x01\x00\x01\x00\xf0\x01\x00\xff\xff\xff\x00\x00\x00\x21\xf9\x04\x01\x0a\x00\x00\x00\x2c\x00\x00\x00\x00\x01\x00\x01\x00\x00\x02\x02\x44\x01\x00\x3b', 'binary');		
						...
						res.writeHead(200, {'Server': 'Kurunt', 'Content-Type': 'application/json; charset=utf-8', 'Transfer-Encoding': 'chunked', 'Connection': 'keep-alive'});
						res.write( JSON.stringify({"uid": id.uid}) + '\n' );   // return the message id.
						res.end();
					}
				} else {
					log(processID+'@inputs> No client host access found for ' + req.headers.host + ' on apikey: ' + apikey + ' will ignore client.');
					res.writeHead(403, {'Server': 'Kurunt', 'Content-Type': 'application/json; charset=utf-8', 'Connection': 'closed'});
					res.write( JSON.stringify({"status": "unauthorized request"}) + '\n' );   // return the message id.
					res.end();
					return;
		...
		req.connection.addListener('close', function () {
		  console.log('http@inputs> Client closed. Last request for: ' + req.url + ' by ip: ' + client_ip);
		});
