...
function init(_pid, topology) {
	...
	for ( var c = 0; c < topology['connections'].length; c++ ) {
		...
		log('messenger:'+processID+'@inputs> zmq_pattern: ' + zmq_pattern );
		
		
		mq_sockets[c]['socket'] = mq.socket(zmq_pattern);
		if ( topology['connections'][c]['zmq_socket'] == 'bind' ) {
			log('messenger:'+processID+"@inputs> zmq binding for apikey: " + apikey + ", namespace: " + namespace + ", at: " + zmq_pattern + "@" + zmq_address);
			mq_sockets[c]['socket'].bind(topology['connections'][c]['zmq_address']);
		} else {
			log('messenger:'+processID+"zmq connecting for apikey: " + apikey + ", namespace: " + namespace + ", at: " + zmq_pattern + "@" + zmq_address);
			mq_sockets[c]['socket'].connect(topology['connections'][c]['zmq_address']);	
		}
		
		mq_sockets[c]['socket'].on('disconnect', function(s){
			log('messenger:'+processID + '@input> ' + "disconnected: " + s._peername.port);
...
/*
		...
		mq_sockets[c]['socket'].on('drop', function(msg){
			log(processID + '@input> HWM start saving to disk - n: ' + n + ' messages.length: ' + messages.length + ', bytes: ' + buffer_size  + ' qu: ' + mq_sockets[0]['socket'].queue.length);
			// save to disk.
...
*/
	...
	setInterval(function () {		
		...
		log(processID + '@inputs> mps: ' + mps + ' n: ' + n + ' qu: ' + mq_sockets[0]['socket'].queue.length, true);
		mps = 0;		// reset
	}, 1000);
...
function push(apikey, message, id, mime_type) {
...
function send(m, apikey, buffer_size, timedOut) {
	log('messenger:http@inputs> send');

	try {
		if ( streams[apikey].status === 'closed' ) {
			log('messenger:tcp@inputs> stream closed dont send');
			return false;
		}
	} catch(e) {
		return false;
	...
	for ( var c in mq_sockets ) {
		log(processID + '@inputs> sending msg to wire! apikey: ' + apikey + ' timedOut: ' + timedOut + ' m.len: ' + m.length + ' idb: ' + idb);
