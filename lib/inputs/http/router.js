...
module.exports.route = function (apikey, input, client_host, message, id, mime_type, messenger, streams, callback) {
	...
	if ( access_hosts.length === 0 ) {
		...
		for ( var a in access_hosts ) {
			if ( access_hosts[a] === '' ) { 		// allow any host.
				allow = true;
				break;
			} else if ( client_host === access_hosts[a] ) {			// allowed host.
				allow = true;
				break;
			}
		}
	...
	if ( allow === false ) {
		...
		messenger(apikey, message, id, mime_type);		// calls messenger.push function.
