...
process.on('message', function(m) {
	console.log('tcp, process.on.message>' + util.inspect(m, true, 99, true));
	if ( m.id !== undefined ) {
		processID = 'tcp#' + m.id;
	}
...
function _init(topology) {
	...
	*/
	...
	messenger.init(processID, topology);	


	if ( config['open_inputs_via_admin'] === true ) {
		console.log(processID + "@inputs> TCP (input) open_inputs_via_admin");
	} else {
		startServer(data);
	}	
