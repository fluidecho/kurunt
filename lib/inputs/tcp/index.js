...
var log 				= function(txt, benchmarking) {
	if ( gconfig['loging'] === 'benchmarking' ) {
...
process.on('message', function(m) {
	log(processID + "@inputs>, process.on.message: " + util.inspect(m, true, 99, true));
...
function startServer(pid, streams) {
	if ( streams == undefined ) {
		log(processID + "@inputs> TCP (input) '" + config['name'] + "' has no streams to open, goodbye.");
...
function close_clients(apikey) {
	...
	for (var c in server.clients) {
		if ( server.clients[c]['apikey'] === apikey ) {
			log(processID + "@inputs> closing client connected on apikey: " + apikey + ".");
			try {
