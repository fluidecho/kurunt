...
my $remote_host = '127.0.0.1';		# change to remote address of tcp server.
my $remote_port = 9333;				# change to remote port of tcp server.

my $socket = IO::Socket::INET->new(PeerAddr => $remote_host,
                                PeerPort => $remote_port,
                                Proto    => "tcp",
                                Type     => SOCK_STREAM)
    or die "Couldn't connect to tcp server at $remote_host:$remote_port : $@\n";

# enter a message. note include duid.
