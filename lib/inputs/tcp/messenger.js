...
*/
	...
	setInterval(function () {		
		...
		console.log(processID + '@input> mps: ' + mps + ' n: ' + n + ' qu: ' + mq_sockets[0]['socket'].queue.length);
		mps = 0;		// reset
	}, 1000);
	...
	for (i = 0; i < bufferLoop.length; i++) {
  		...
  		if ( bufferLoop.readUInt8(i) == lconfig["message_delineate"] && i != (esc + 1) ) {
  			//console.log('is delineate, at: ' + i + ' x: ' + x + ' messages.length: ' + messages.length);
				...
				messages[apikey].push(m);		// push message into array by apikey (worker).
				...
				if ( lconfig['mq_nodelay'] === true ) {
					...
					send(msgs, apikey, worker, stores, tags, buffer_size[apikey], false);
					...
					if ( messages[apikey].length === 1 ) {
						...
						nagleishTimeOut[apikey] = (function () {
							setTimeout(function () {
								//console.log('nagleishTimeOut!, apikey: ' + apikey);
								if ( buffer_size[apikey] > 0 ) {
									...
									messages[apikey] = [];
									buffer_size[apikey] = 0;
									send(msgs, apikey, worker, stores, tags, buffer_size[apikey], true);				// send.
								}
							}, TCP_TRAIL_EXPIRES);
						})();	
					} 			
					...
					if ( buffer_size[apikey] > TCP_BUFFER_SIZE ) {
							...
							messages[apikey] = [];
							buffer_size[apikey] = 0;
							send(msgs, apikey, worker, stores, tags, buffer_size[apikey], false);							// send.
					}
