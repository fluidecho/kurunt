...
function push(apikey, chunk) {
	...
	if ( buffer.length > lconfig['chunk_buffer_limit'] ) {
		...
		log('messenger:tcp@inputs> Warning: Maximum chunk buffer size limit exceeded, dropping buffer.');
		buffer = null;
		return false;
	}
...
function send(m, apikey, buffer_size, timedOut) {
	...
	log('messenger:tcp@inputs> apikey: ' + apikey, false, streams[apikey]);
