...
function _load(topology) {
	...
	setInterval(function () {		
		//debugger;
		console.log('tcp@input> mps: ' + mps + ' n: ' + n + ' messages.length: ' + messages.length + ', bytes: ' + buffer_size  + ' qu: ' + mq_sockets[0]['socket'].queue.length);
		mps = 0;		// reset
	}, 1000);
...
function push(apikey, worker, stores, tags, client, chunk) {
  		...
  		if ( bufferLoop.readUInt8(i) == config["message_delineate"] && i != (esc + 1) ) {
  			//console.log('is delineate, at: ' + i + ' x: ' + x + ' messages.length: ' + messages.length);
				...
				if ( config['mq_nodelay'] === true ) {
					...
					send(msgs, apikey, worker, stores, tags, client, buffer_size, false);
						...
						nagleishTimeOut = (function () {
							setTimeout(function () {
								if ( buffer_size > 0 ) {
									...
									messages = [];
									buffer_size = 0;
									send(msgs, apikey, worker, stores, tags, client, buffer_size, true);				// send.
								}
							}, TCP_TRAIL_EXPIRES);
						})();	
					} 			
					...
					if ( buffer_size > TCP_BUFFER_SIZE ) {
							...
							messages = [];
							buffer_size = 0;
							send(msgs, apikey, worker, stores, tags, client, buffer_size, false);							// send.
					}
...
function send(m, apikey, worker, stores, tags, client, buffer_size, timedOut) {
