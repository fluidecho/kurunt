...
function push(apikey, chunk) {
	...
	for (i = 0; i < bufferLoop.length; i++) {
  		...
  		if ( bufferLoop.readUInt8(i) == lconfig["message_delineate"] && i != (esc + 1) ) {
  			logging.log('tcp@inputs> is delineate, at: ' + i + ' x: ' + x + ' messages.length: ' + messages.length + ', for apikey: ' + apikey);
				...
				logging.log('tcp@inputs> m: ' + m.toString());
				
				logging.log('tcp@inputs> m.length: ' + m.length);
				if ( m.length === 0 ) {
					// cant send a blank event, return nada.
					logging.log('tcp@inputs> cant send a blank event, return false');
					return false;
				}
				...
				logging.log('tcp@inputs> buffer_size[apikey]: ' + buffer_size[apikey]);
				if ( gconfig['mq_nodelay'] === true ) {
					...
					if ( messages[apikey].length === 1 ) {
						logging.log('tcp@inputs> nagleishTimeOut');
						// start nagleish delay timmer.
						nagleishTimeOut[apikey] = (function () {
							setTimeout(function () {
								logging.log('tcp@inputs> nagleishTimeOut callbacked');
								if ( buffer_size[apikey] > 0 ) {
									logging.log('tcp@inputs> nagleishTimeOut callbacked, if buffer_size[apikey] > 0');
									var msgs = messages[apikey];
