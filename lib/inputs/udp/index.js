...
// UDP Input
//
// UDP input server for tcp clients like syslog.
// Version: 0.2
...
process.on('SIGINT', function() {
	log(processID + '@inputs> SIGINT, exit.');
  process.exit(code=0);
});



var processID = 'udp#' + process.pid;
var pid = 0;

// listen for topology.js to set config and topology then load this.
process.on('message', function(m) {
	log(processID + "@inputs>, process.on.message: ", false, m);
...
function _init(pid, topology) {
	...
	log(config['name'] + '#' + pid + '@inputs> started.');
...
function startServer(pid, streams) {
	if ( streams == undefined ) {
		log(processID + "@inputs> UDP (input) '" + config['name'] + "' has no streams to open, goodbye.");
	...
	for ( var x = 0; x < streams['streams'].length; x++ ) {
		if ( streams['streams'][x]['status'] != 'closed' && streams['streams'][x]['input']['object'] === config['name'] && streams['streams'][x]['input']['id'] === pid ) {
			...
			log(processID + "@inputs> Opening stream with apikey: " + streams['streams'][x]['apikey'] + " on port: " + streams['streams'][x]['apikey']);
	...
	process.send({ object: 'input', namespace: 'udp', message: 'loaded' });
}
...
function newStream(stream) {
	...
	if ( stream['status'] === 'open' && stream['input']['object'] === config['name'] && stream['input']['id'] === pid ) {	
		...
		log(processID + "@inputs> Opening stream with apikey: " + stream['apikey'] + " on port: " + stream['apikey']);
		server.start(processID, stream['apikey'], router.route, messenger.push, messenger.streams);
		log(processID + '@inputs> messenger.streams: ' + util.inspect(messenger.streams, true, 99, true));
	}
}
...
function deleteStream(apikey) {
	log(processID + "@inputs> deleteStream is deleting apikey: " + apikey + ".");
	try {
...
function close_stream(apikey) {
	log(processID + "@inputs> closing server (stream) using apikey: " + apikey + ".");
	try {
