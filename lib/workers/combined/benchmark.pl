...
if ( $h ne '' || $help ne '' ) {
	...
	print "For example> perl benchmark.pl -T=tcp -P=6001 -m=100 -c=10 -d='127.0.0.1 - frank [10/Oct/2000:13:55:36 -0700] \"GET /apache_pb.gif HTTP/1.0\" 200 2326 \"http://www.example.com/start.html\" \"Mozilla/4.08 [en] (Win98; I ;Nav)\" \"USERID=Zepheira;IMPID=01234\"'\n";
	print "This example would send 100 messages every second for 10 seconds through port 6002, -H (host), -d (data).\n";
	exit(0);
}
if ( $P eq '' ) {
