...
process.argv.forEach(function(val, index, array) {
	...
	if ( val.substring(0, 9) === '-workers=' ) {
		...
		console.log('workers> load these workers: ' + workers_asmodules);
		console.log('workers> workers: ' + require('util').inspect(workers_asmodules, true, 99, true));
	}
...
function _loadAllWorkers() {
	...
	for ( var wk in workers_asmodules ) {
		...
		console.log('loadAllWorkers: ' + wk + ' - ' + worker_file);
	...
	console.log('index@workers> _loadAllWorkers, workers: ' + util.inspect(workers, true, 99, true));
		...
		dirs.forEach(function(worker){
			...
			if (stats.isDirectory()) {
				...
				if ( worker.substring(0, 1) != '_' ) {
					...
					console.log('worker to load: ' + workers[worker]['config']['title']);
