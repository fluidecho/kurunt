...
process.on('SIGINT', function() {
	log(processID + '@workers> SIGINT, exit.');
  process.exit(code=0);
});
...
process.on('message', function(m, func) {
	log('*workers, process.on.message>', false, m);
...
process.argv.forEach(function(val, index, array) {
	...
	if ( val.substring(0, 9) === '-workers=' ) {
		...
		log('workers> load these workers: ' + workers_asmodules);
		log('workers> workers: ', false, workers_asmodules);
	}
...
function _init(pid, config, topology) {
	...
	log(processID + "@workers> initiating.");
...
_loadAllWorkers();
	...
	setInterval(function () {		
		log(processID + '@workers> mps: ' + mps + ' total: ' + tot, true);
		mps = 0;		// reset
	}, 1000);	
...
function _loadAllWorkers() {
	...
	for ( var wk in workers_asmodules ) {
		...
		//log('loadAllWorkers: ' + wk + ' - ' + worker_file);
	...
	//log('index@workers> _loadAllWorkers, workers: ', false, workers);
		...
		dirs.forEach(function(worker){
			...
			if (stats.isDirectory()) {
				...
				if ( worker.substring(0, 1) != '_' ) {
					...
					log('worker to load: ' + workers[worker]['config']['title']);
