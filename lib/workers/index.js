...
var fn = undefined;		// functions to pass to workers.
var cf = {};		// configs to pass to workers.
...
function _loadAllWorkers() {
	...
	for ( var wk in workers_asmodules ) {
		...
		if (typeof workers[worker_config.name]['module'].init === 'function') {
			workers[worker_config.name]['module'].init(processID, cf, fn);		// load init worker function if exists.
		}	
			...
			if (stats.isDirectory()) {
				...
				if ( worker.substring(0, 1) != '_' ) {
					...
					if (typeof workers[worker]['module'].init === 'function') {
						workers[worker]['module'].init(processID, cf, fn);		// load init worker function if exists.
					}					
