...
module.exports.work = function (message, wk, fn, callback) {
  ...
  fn.logging.log('json@workers> MESSAGE: ' + require('util').inspect(message, true, 99, true));    // uncomment to debug message.
  ...
  try {
    ...
    var json = JSON.parse(message.message.toString(wk['config']['encoding']));
		...
		fn.logging.log('json@workers> json: ' + require('util').inspect(json, true, 99, true));
    ...
    fn.logging.log('json@workers> ERROR: ' + require('util').inspect(e, true, 99, true));     // uncomment to debug errors.
