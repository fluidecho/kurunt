...
if ( $h ne '' || $help ne '' ) {
	...
	print "   -P = 7777 (number, required command, port number to Kurunt tcp or udp input port)\n";
	print "   -H = 127.0.0.1 (ip address, optional command, ip address to Kurunt tcp or udp input host)\n";
	print "   -d = data (json, optional command, data to send)\n";	
	print "   -m = 100 (number, required command, of messages to send)\n";
	print "   -c = 10 (number, required command, of cycles to send messages)\n\n";
	print "For example> perl client.pl -T=tcp -P=7777 -m=100 -c=10 -d='{\"hello\":\"world\"}'\n";
	print "This example would send 100 messages every second for 10 seconds through port 7777, -H (host), -d (data) {\"hello\":\"world\"}.\n";
	exit(0);
}
if ( $P eq '' ) {
...
# default sending messages in json format.
if ( $d eq '' ) {
	$d = '{"hello":"world"}';
}
