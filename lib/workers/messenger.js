...
function init(xconfig, Connections) {
	...
	log('messenger@workers, connections> ', false, connections);
	var c = 0;
	for ( c = 0; c < connections.length; c++ ) {
		log('messenger@workers> make ZMQ this connection for socket#' + c);
		...
		log('messenger@workers> zmq_pattern: ' + zmq_pattern );		
		...
		log('messenger@workers> zmq_address: ' + connections[c]['zmq_address']);
		if ( connections[c]['zmq_socket'] === 'bind' ) {
 			socks[c].bind(connections[c]['zmq_address']);
 		} else {
 			socks[c].connect(connections[c]['zmq_address']);
 		}
		log('messenger@workers> messenger, ' + connections[c]['zmq_socket'] + ':' + zmq_pattern + '@' + connections[c]['zmq_address']);		
		...
		socks[c].on('connect', function(a){
			...
			log('messenger@workers> CONNECTED -------------- socket#' + thisc);
...
//console.log('worker A msg> ' + util.inspect(msg, true, 99, true));
