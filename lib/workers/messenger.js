...
function init(xconfig, connections) {
	...
	//sock_stream	= mq.socket('push');
	//sock_stream.connect('tcp://127.0.0.1:3055');
	//console.log('*@workers> connected to stream: tcp://127.0.0.1:3055');	
	...
	for ( c = 0; c < connections.length; c++ ) {
		...
		console.log('zmq_address: ' + zmq_pattern);
		...
		socks[c].on('message', function(msg, reply){
			...
			try {
				...
				for ( i = 0; i < msg.messages.length; i++ ) {
					...
					//message._peername	= msg._peername;
					...
					// pause upstream
					//console.log('_this> ' + util.inspect(_this, true, 99, true));
					//_this.socks[0].pause();
		...
		var nagleishTimeOut = (function () {
			setTimeout(function () {
				if ( batched_msg !== '' ) {
					...
					//var m_temp = batched_m;
					//batched_m = [];
					//var msg = msgpack.encode(m_temp);					
					//var msg = m_temp;
