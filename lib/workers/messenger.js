...
//config['mq_nodelay'] = true;
	...
	if ( config['mq_nodelay'] === true ) {
		...
		var s = 0;
		for ( s = 0; s < connections.length; c++ ) {
			if ( connections[s]['object'] === 'store' ) {
				console.log('workers@messenger> send to store - ' + connections[c]['zmq_socket'] + ':' + connections[c]['zmq_pattern'] + '@' + connections[c]['zmq_address']);
