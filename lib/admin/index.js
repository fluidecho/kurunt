...
// get inputs/tcp open port range
var tcp_config = require('../inputs/tcp/./config.json');
console.log('tcp_config, open_port_start: ' + tcp_config['open_port_start']);
var last_tcp_port = Number(tcp_config['open_port_start']);

var last_udp_port = 7001;
...
if ( config['admin_www'] === true ) {
								...
								req.on("end", function() {
									...
									if ( postdata.newStream === 'true' || postdata.editStream === 'true' ) {		// NOTE: is 'true' literal not js true!!!
										...
										if ( postdata.newStream === 'true' ) {
											...
											if ( postdata.input === 'tcp' ) {
												last_tcp_port++;
												apikey = last_tcp_port;										
											} else if ( postdata.input === 'udp' ) {
												last_udp_port++;
												apikey = last_udp_port;
											} else {
										...
										fs.writeFile(streamfile, JSON.stringify(streams), function (err) {
											...
											// send new/edit stream command to input !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! NOW....
											callback();	
										});					
							...
							fs.readFile(__dirname + '/../../streams.json', function (err, s) {
								...
								try {
									...
									for ( var sx in st['streams'] ) {
										...
										if ( st['streams'][sx]['input']['object'] === 'tcp' ) {
											if ( Number(st['streams'][sx]['apikey']) > last_tcp_port ) {
												last_tcp_port = Number(st['streams'][sx]['apikey']);
											}
										} else if ( st['streams'][sx]['input']['object'] === 'udp' ) {
											if ( Number(st['streams'][sx]['apikey']) > last_tcp_port ) {
												last_udp_port = Number(st['streams'][sx]['apikey']);
											}
										} else {
										}
										hasStream = true;
	...
	console.log("web admin running at\n  => http://" + config['host'] + ":" + config['admin_www_port'] + "/\nCTRL + C to shutdown");
} else {
