...
if ( config['admin_www'] === true ) {
								...
								req.on("end", function() {
									...
									if ( postdata.sendData === 'true' && postdata.apikey != undefined ) {
										...
										sendData.send(postdata.apikey, function(msent) {
											log('msent to apikey: ' + postdata.apikey + ' message: ' + msent);
	...
	log("Open your web browser to admin at => http://" + config['host'] + ":" + config['admin_www_port'] + "/\n");
} else {
	log("web admin not running.");
}
