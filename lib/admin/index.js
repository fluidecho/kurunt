...
if ( config['admin_www'] === true ) {
  ...
  if ( module.parent.parent.parent.filename.substring(module.parent.parent.parent.filename.length - 6) === 'cli.js' ) {
    console.log("Open your web browser to admin at => http://" + config['host'] + ":" + config['admin_www_port'] + "/\n");    // called standalone, use console.log
  } else {
    logging.log("Open your web browser to admin at => http://" + config['host'] + ":" + config['admin_www_port'] + "/\n");    // called as module, use logging
  }
} else {
...
function getEC2PublicAddress() {

	// question> how to check if this enviromnet is ec2 to start with, before checking address which can take several seconds.
